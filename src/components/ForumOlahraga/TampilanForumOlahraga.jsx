...
const TampilanForumOlahraga = () => {
  const [data, setData] = useState([]);
  const { id } = useParams();
  const [olahragas, setOlaharaga] = useState([]);
  ...
  const getForumByKategori = async (id) => {
    try {
      ...
      setOlaharaga(response.data);
    } catch (error) {
      console.log(error);
    }
  };

  console.log("Forum", olahragas);
    ...
    <div className="flex-col ">
      ...
      <div className="  bg-red-400  items-center ">
        <div className="flex flex-col">
          <h1 className="text-xl text-center ">{olahragas.nama_forum}</h1>
          <img
            src={olahragas.URL}
            alt=""
...
export default TampilanForumOlahraga;
