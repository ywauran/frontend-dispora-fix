...
const SuratKadis = () => {
  ...
  const getSuratKadis = async () => {
    const response = await axios.get(
      `http://localhost:4000/suratkadis?search_query=${keyword}&page=${page}&limit=${limit}`
    );
    setPage(response.data.page);
    setPages(response.data.totalPage);
    setRows(response.data.totalRows);
    setSuratkadis(response.data.result);
  ...
  const deleteSuratKadis = async (id) => {
    try {
            ...
            <div className="flex-col ">
              ...
              <div className="overflow-x-auto my-11">
                <table className="table w-[700px] justify-center  border ml-11 border-black">
                  ...
                  <tbody>
                    {suratkadis.map((item, index) => (
                      <tr key={item.uuid}>
                        <td className="p-4">{generateRowNumber(index)}</td>
                        <td className="p-4">{item.nama_surat_kadis}</td>
                        <td className="p-4">{item.tanggal_surat_kadis}</td>
                        <td className="p-4">
